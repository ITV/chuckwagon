...
object AWSFindSubnets extends AWSService[FindSubnetsRequest, FindSubnetsResponse] {
  ...
  override def apply(findSubnetsRequest: FindSubnetsRequest): FindSubnetsResponse = {
    ...
    val userFilters: List[AWSFilter] = findSubnetsRequest.filters.map {
    ...
    val awsSubnets: List[AWSSubnet] = ec2.describeSubnets(describeSubnetsRequest).getSubnets.asScala.toList
